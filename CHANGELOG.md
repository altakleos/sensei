...
- test: Tier-1 transcript fixture for `protocols/hints.md` at `tests/transcripts/hints.md` (fixtures file) + `tests/transcripts/hints.dogfood.md` (synthetic seed, `status: seed`, `agent: synthetic-seed`). Four fixtures: `triage-not-teaching` (no teaching-mode leakage during triage), `concise-summary` (counts + outcomes, not per-item commentary), `boost-visibility` (mentor names boosted topics — closes ADR-0018's subtle-influence gap), `ambiguity-classification` (unclassifiable hints surface a clarification ask rather than being silently dropped — closes ADR-0019's classify-at-triage bet). Real capture replaces the seed at the next release per `docs/design/transcript-fixtures.md` § Cadence. Hints-spec frontmatter drops `fixtures_deferred:` in favour of a proper `fixtures:` entry.
- docs: graduate ADR-0017 (file-drop ingestion), ADR-0018 (curriculum boosting over rewriting), and ADR-0019 (universal inbox over typed drop zones) from `provisional` back to `accepted`. First end-to-end exercise of the `provisional → evidence → accepted` round-trip documented in `docs/decisions/README.md` § Status values. Each ADR's body replaces its provisional-rationale callout with a graduation note citing the specific `tests/transcripts/hints.md` fixture that closed its review trigger.
