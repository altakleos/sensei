...
- `pacing.py` script — computes recency-weighted learning velocity and projects goal completion date. Status protocol now reports pace (ahead/on-track/behind) when a goal has a deadline.
- New config keys: `pacing.recency_decay` (default 0.7), `pacing.review_overhead_cap` (default 0.3).
