...
Both hints are scoped to the same Rust learning goal the `test_goal_protocol_e2e.py` fixture sets up, so the protocol should classify them into the learner's active curriculum topics (`ownership`, `async-await`) and register them in `learner/hints/hints.yaml`.
