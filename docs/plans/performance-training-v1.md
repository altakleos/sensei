...
Implements the performance training phase overlay for stages 1–4 (learn → automate → verbalize → time pressure). The phase is a context-layer overlay on the existing four-mode model, not a fifth mode. Follows the single phase protocol composition from ADR-0021: one file (`protocols/performance-training.md`) loaded alongside the active mode when the phase is active for the current goal.
...
- [x] T5: Create `protocols/performance-training.md` — the single phase protocol file per ADR-0021. Structure:
