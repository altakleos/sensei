...
- [x] T3: Create `mutate_graph.py` — validated graph mutations (activate, complete, collapse, spawn, expand) → `src/sensei/engine/scripts/mutate_graph.py`
...
- [x] AC4: `mutate_graph.py spawn` rejects mutations that would introduce cycles
- [x] AC5: `mutate_graph.py expand` replaces node with subgraph, original becomes expanded
