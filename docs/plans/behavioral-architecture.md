...
- [x] T12: Update existing specs with new `realizes:` references where the behavioral-modes spec creates new linkable principles. Specifically: review-protocol.md gains `P-principles-not-modes` if appropriate. → `docs/specs/review-protocol.md`
- [x] T13: Run `ci/check_foundations.py` — confirm 0 errors. New principles may show as orphan warnings until Phase 2 specs land; that's expected. → verify
- [x] T14: Full test suite green. → verify
- [x] T15: Append Unreleased entry to `CHANGELOG.md` covering the behavioral-architecture decomposition. → `CHANGELOG.md`

## Acceptance Criteria

- [x] AC1: Two new foundation principles exist: `P-principles-not-modes` (technical) and `P-two-failure-prerequisite` (pedagogical), both with `status: accepted`.
- [x] AC2: `docs/specs/behavioral-modes.md` exists with status: accepted, covering the four modes, transitions, silence profiles, forbidden language, assessor exception, and adaptive behaviors.
- [x] AC3: `docs/design/behavioral-modes.md` exists covering composition strategy, transition triggers, and signal→response table.
- [x] AC4: ADR-0013 (context composition) exists and is indexed.
- [x] AC5: `docs/specs/assessment-protocol.md` exists with status: accepted, distinguishing diagnostic from summative assessment.
- [x] AC6: `docs/specs/performance-training.md` exists with status: draft (v2 feature).
- [x] AC7: `ci/check_foundations.py` passes with 0 errors on the full repo.
- [x] AC8: Full test suite green.
- [x] AC9: CHANGELOG.md `[Unreleased]` describes the change.
