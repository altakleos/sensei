...
- [x] T6: Add a one-paragraph note to `AGENTS.md` recommending Conventional Commits (as loose convention, not enforced) and telling contributors to append user-visible changes to `## [Unreleased]` in the same commit that introduces them. → `AGENTS.md`

### Phase 2 — Verify

- [x] T7: `pytest` full suite green including new changelog checks.
- [x] T8: Local wheel + sdist build — confirm `CHANGELOG.md` appears in the sdist but NOT in the wheel. Run `python ci/check_package_contents.py --wheel dist/*.whl --tag v0.1.0a1` against a wheel matched by the existing seeded changelog entry; expect status `ok`.
- [x] T9: Smoke — bump version to a throwaway `v0.1.0a99` in a local branch, rebuild, run the validator; expect exit 4 (`changelog_missing_entry`). Do NOT commit or tag the throwaway bump.

## Acceptance Criteria

- [x] AC1: `CHANGELOG.md` exists, passes a manual Keep-a-Changelog format read, has `## [Unreleased]` and `## [0.1.0a1] — 2026-04-20`.
- [x] AC2: `pyproject.toml` sdist include lists `CHANGELOG.md`; wheel target does not force-include it.
- [x] AC3: `ci/check_package_contents.py` exits 4 when the tag's version has no dated entry; exits 0 for a valid full pipeline on `v0.1.0a1`.
- [x] AC4: `tests/ci/test_check_package_contents.py` covers changelog-missing-entry, malformed-entry, and unreleased-only scenarios.
- [x] AC5: Playbook's pre-release checklist enforces the Unreleased → version promotion; playbook Yank section documents the amendment rule; References point to the new spec.
- [x] AC6: `AGENTS.md` mentions Conventional Commits preference and the "append to Unreleased" rule.
- [x] AC7: Suite count grows by ≥3 tests.
