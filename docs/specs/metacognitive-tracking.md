...
- **Calibration accuracy is tracked over time.** The ratio of confidence-correct responses to total confident responses is computed from the confidence quadrant history. A learner who is confident and correct 9 out of 10 times has calibration_accuracy 0.9. Null means unknown (insufficient data). *Implementation note: the current implementation computes overall accuracy (correct / attempts) as a proxy. True calibration (confident-correct / total-confident) requires per-response confidence tracking, which is deferred.*
