...
P-emotion-cognition-are-one establishes that emotional state is learning infrastructure, not a side effect. The principle's degradation chain (confusion → frustration → boredom → disengagement) identifies the specific dynamic the system must monitor. D'Mello & Graesser's affect dynamics model [Bibliography #59, #60] provides the empirical grounding: these four emotions are the ones that matter for learning, and tracking transitions between them is more informative than snapshots.
